...
(async () => {
     ...
     못 잡는 것은 docs/규칙.md 규칙 8 에 적어 두었다. */
  {
    ...
    /* ── 그물은 **db/name_pattern.json 한 곳**에 있다 ──
       전에는 이 검사와 tools/link-case.mjs 가 각자 갖고 있었다. link-case 쪽에
       `변호인` 이 빠져 있어서 「변호인 선임권」이 담긴 판시사항이 그대로 올라가고
       이 검사가 그것을 FAIL 로 잡았다. **거르는 쪽과 재는 쪽이 다르면 갈라진다.** */
    const NP = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'db', 'name_pattern.json'), 'utf8'));
    const SUR = NP.sur;
    const JOSA = NP.josa;
    const ROLE = NP.role;
    const HARD = [
     ...
     그래서 둘을 함께 잰다 — **그린 결과의 수**와 **나선기(황금각 비율)**. */
  {
    ...
    if (!r) F('65. 못 쟀다');
      ...
      /* ── 한도를 **절대값**으로 박으면 노드가 늘 때마다 흔들린다 ──
         표준편차는 노드 수가 늘면 같은 고르기에서도 √N 만큼 커진다.
         실제로 20개에서 통과하던 배치가 45개가 되자 1.09 로 FAIL 했다 —
         방향은 [5,4,3,4,3,5,6,2,4,3,3,3] 로 **더 고른데도** 그랬다.
