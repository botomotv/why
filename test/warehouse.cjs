   ...
   그래서 예외로 두되 **이름을 박아 둔다** — 예외가 늘어나면 그때 다시 본다. */
{
  ...
  const EXPECTED_EMPTY = { r3: '방첩사 개편은 대통령령(직제)이라 법안명에 흔적이 없다',
                           ...
                           /* 「사이버」가 든 법안 15건 중 **14건이 공포되지 않았다** —
                              사이버안보 기본법은 20년째 발의만 되고 전부 폐기됐다.
                              공포된 하나는 2012년 특검법이라 주제가 다르다.
                              이을 법이 없는 것이 아니라 **통과한 법이 없다.** */
                           ix_cyber: '사이버안보 법안 15건 중 14건이 공포되지 않았다 (발의 후 폐기)' };
  if (!fs.existsSync(WH)) {
